package com.creationalPatterns.abstractFactory.example2;
...
class PizzaFactory {
    static Pizza getPizza(String type, ToppingFactory toppingFactory) {
        ...
        Pizza pizza = switch (type.toLowerCase()) {
            case "cheese" -> new CheesePizza(ingredients);
            case "pepperoni" -> new PepperoniPizza(ingredients);
            case "veggie" -> new VegetarianPizza(ingredients);
...
class ToppingFactoryFactory {
    static ToppingFactory getToppingFactory(String type) {
        ...
        return switch (type.toLowerCase()) {
            case "standard" -> new StandardToppingFactory();
            case "mexican" -> new MexicanToppingFactory();
            case "turkish" -> new TurkishToppingFactory();
            default -> throw new IllegalArgumentException("No such pizza.");
        };
